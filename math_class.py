...
class Foot(pygame.sprite.Sprite):
    ...
    def pos_update(self,move_distance, move_distance_vertical):
        if abs(move_distance) > 0.002:
            self.rect.left -= move_distance * move_rate
        if abs(move_distance_vertical) > 0.003:
            self.rect.top += move_distance_vertical * move_rate_vertical
